...
class trainModel:
    ...
    def trainingModel(self):
        ...
        try:
            ...
            data = preprocessor.dropUnnecessaryColumns(data, ['id', 'region', 'url', 'region_url', 'image_url', 'state', 'dogs_allowed', 'type'])

            #removing outliers from columns like height, weight, ap_hi, ap_lo
            data = preprocessor.dropOutliers(data)

            # get encoded values for categorical data and impute missing values from both categorical and non-categorical features

            data = preprocessor.encodeCategoricalValues(data)

            # create separate features and labels
            X,Y=preprocessor.separate_label_feature(data,label_column_name='price')
